# Hey there! 👋 I'm Lê Triều Phương Hiếu

**Full-Stack Engineer | AI & IoT Enthusiast | Award-Winning Innovator**

I'm a final-year Information Technology student at Thu Dau Mot University, fueled by a passion for crafting innovative software solutions that solve real-world problems. Whether it's building responsive web apps, designing AI-driven systems, or integrating IoT for smarter living, I thrive at the intersection of code, creativity, and impact.

🔭 **Currently working on:** Leading the charge to establish an AIoT Club at my university—stay tuned for some groundbreaking projects in 2025!  
🌱 **Learning:** Diving deeper into cloud-native development and advanced machine learning models.  
💬 **Ask me about:** Full-stack dev, AI algorithms, or my journey from competition wins to research papers!  
⚡ **Fun fact:** I once turned a greenhouse into a smart ecosystem—plants and tech make a great team!

---

## 🚀 What I Bring to the Table

- **Languages:** JavaScript, Python, Java, C#
- **Frontend:** ReactJS
- **Backend:** Node.js, Express, ASP.NET MVC
- **Databases:** MongoDB, Firebase, SQL
- **Cloud & Tools:** AWS, Advantech Cloud, Docker, Kubernetes, Netlify, Render
- **Specialties:** AI (CNNs, pre-trained models), IoT, web development, research

I’ve led teams to victory in competitions like the **TDMU Entrepreneurship Competition 2024 (Champion)** and earned accolades like the **Best Marketing Strategy Award** at H-BRIDGE Global CDP 2025. My projects range from skin cancer detection AI to encrypted chat apps—check them out below!

---

## 🏆 Achievements That Light My Fire

- **Quán quân (Champion)** – TDMU Entrepreneurship Competition 2024: Built a "Smart Greenhouse Monitoring Solution" to revolutionize sustainable farming.
- **Giải Nhất (First Prize)** – TDMU Scientific Talent Award 2024: Recognized for standout research among 100+ candidates.
- **Best Marketing Strategy Award** – H-BRIDGE Global CDP 2025 (Hannam University, Korea): Pioneered satellite-based rice yield forecasting.
- **Published Author** – 2 Scopus-indexed papers at ICDAM 2024: Tackled COVID-19 detection and skin cancer accuracy with AI.
- **Semi-Finalist** – Euréka Science Prize 2024 & AIoT Developer Innoworks 2024: Pushed boundaries in research and tech innovation.

---

## 🌟 Featured Projects

- **[Andrew Blog](https://andrew-le-blog.netlify.app/)**  
  A sleek, responsive blogging platform built with ReactJS, Node.js, and MongoDB. Hosted on Netlify and Render—perfect for sharing ideas and experiences.  
  _"Code is my canvas, and this is my story."_

- **[Skin Cancer Detection](https://github.com/hieultph/skin-cancer-detection)**  
  An AI-powered website (ReactJS + AWS) to detect skin cancer from images. Research-backed and user-friendly—bridging tech and healthcare.  
  _"Small pixels, big impact."_

- **[Encrypted Chat App](https://github.com/hieultph/encrypted-chat-app)**  
  A secure, real-time chat app with Firebase and ReactJS. Privacy-first design with a sprinkle of emojis!  
  _"Talk safe, vibe freely."_

- **[TDMU Admin Dashboard](https://github.com/hieultph/tdmu-admin)**  
  A full-featured dashboard (ASP.NET MVC, C#) with AI-driven predictive analytics for student performance. Streamlining university ops, one line of code at a time.  
  _"Data-driven decisions, student success."_

---

## 📊 GitHub Stats

![Hieu's GitHub Stats](https://github-readme-stats.vercel.app/api?username=hieultph&show_icons=true&theme=radical)  
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=hieultph&layout=compact&theme=radical)

---

## 🌍 Let’s Connect!

- 📧 Email: [hieultph@gmail.com](mailto:hieultph@gmail.com)
- 🌐 Portfolio: [andrew-le-blog.netlify.app](https://andrew-le-blog.netlify.app)
- 💼 LinkedIn: [hieultph](https://linkedin.com/in/hieultph)
- 🐦 X: [@hieultph](https://x.com/hieultph)

I’m always up for a chat about tech, innovation, or just geeking out over the latest algorithms. Let’s build something amazing together! 🚀

---

_"Code isn’t just about syntax—it’s about solving problems, sparking ideas, and leaving a mark on the world."_
